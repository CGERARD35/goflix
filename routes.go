...
func (s *server) routes() {
	...
	s.router.HandleFunc("/api/movies/{id:[0-9]+}", s.loggedOnly(s.handleDeleteMovieById())).Methods("DELETE")
	s.router.HandleFunc("/api/movies/{id:[0-9]+}", s.loggedOnly(s.handleUpdateMovie())).Methods("PUT")
}
